...
param_table *particleset_param_table(const char *name) {
    ...
    add_param_opt(pt, PAR_PARTICLE_COORDS,
                  "s," PAR_PARTICLE_COORDS__FILE "," PAR_PARTICLE_COORDS__RANDOM "," PAR_PARTICLE_COORDS__GRID);
    add_param_opt(pt, PAR_GEN_RAND_POSITIONS, "b");
    add_param_opt(pt, PAR_WHERE,
                  "s," PAR_WHERE__VOLUME "," PAR_WHERE__SURFACE "," PAR_WHERE__TOP "," PAR_WHERE__BOTTOM);
    ...
    for (k = 0; k < o; k++) {
        for (j = 0; j < n; j++) {
            for (i = 0; i < m; i++) {
                ...
                if ((a == 0 && fabs(trans_pos[2]) > half_edge_thickness) ||
                    (a != 0 && (l1 < (R_sqr) || l2 < (R_sqr)))) {
                    ...
                    set_array_entry(&masked_particle->lap_pot_re, i, j, k,
                                    get_array_entry(&particle_org->lap_pot_re, i, j, k));
                    set_array_entry(&masked_particle->lap_pot_im, i, j, k,
                                    get_array_entry(&particle_org->lap_pot_im, i, j, k));
                }
            }
        }
    }
