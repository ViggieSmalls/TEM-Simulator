    ...
    switch (p->type) {
        ...
        case 's':
            if (NULL == p->allowed) {
                ...
                delim[0] = p->allowed[0];
                delim[1] = '\0'; /* First character in string is delimiter */
...
int check_param(param *p, const char *ptclass, const char *ptname) {
    ...
    switch (p->type) {
        case 'i':
            val = (double) *(int *) p->addr;
            break;
        case 'l':
            val = (double) *(long *) p->addr;
            break;
        case 'f':
            val = (double) *(float *) p->addr;
            break;
        case 'd':
            val = *(double *) p->addr;
            break;
    ...
    for (i = 0; i < P->used; i++) {
        ...
        if (0 == P->params[i].set) {
            ...
            switch (P->params[i].type) {
                case 'c':
                    fprintf(fp, "%-25s = %c", P->params[i].name, *(char *) (P->params[i].addr));
                    break;
                case 'i':
                    fprintf(fp, "%-25s = %i", P->params[i].name, *(int *) (P->params[i].addr));
                    break;
                case 'l':
                    fprintf(fp, "%-25s = %i", P->params[i].name, (int) *(long *) (P->params[i].addr));
                    break;
                case 'f':
                    fprintf(fp, "%-25s = %f", P->params[i].name, *(float *) (P->params[i].addr));
                    break;
                case 'd':
                    fprintf(fp, "%-25s = %f", P->params[i].name, (float) *(double *) (P->params[i].addr));
                    break;
                case 's':
