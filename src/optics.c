...
param_table *optics_param_table(const char *name) {
    // +2 size of param_table (12+2->14)
    param_table *pt = new_param_table(14, TYPE_OPTICS, name);
...
int optics_check_input(optics *o) {
    ...
    if (get_param_boolean(o->param, PAR_GEN_RAND_DEFOCUS)) {
        if (require_param(o->param, PAR_DEFOCUS_NOMINAL) || require_param(o->param, PAR_DEFOCUS_SYST_ERROR)
            || require_param(o->param, PAR_DEFOCUS_NONSYST_ERROR))
            return 1;
    } else {
...
int optics_generate_random_defocus(optics *o, long ntilts) {
    ...
    def_avg = rand_gauss(get_param_double(o->param, PAR_DEFOCUS_NOMINAL),
                         get_param_double(o->param, PAR_DEFOCUS_SYST_ERROR));
...
double optics_get_phase_plate_spot_size(optics *o) {
    return (1 / ONE_NANOMETER) * get_param_double(o->param, PAR_PHASE_SPOT);
}