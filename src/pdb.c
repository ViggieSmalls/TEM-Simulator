...
int add_atomic_pot(array *vol, double xc, double yc, double zc, int atomic_num, double temp_fact, double voxel_size,
                   double num);
...
int get_pot_from_pdb(particle *p, electronbeam *ed) {
        ...
        if (parse_pdb_line(line, &pdbrec)) {
            ...
            if (0 != pdbrec.atomic_num) {
                ...
                for (j = 0; j < transf.size[2]; j++) {
                    ...
                    coord[1] -= box[2];
                    coord[2] -= box[4];
                    add_atomic_pot(&(p->pot_re), coord[0], coord[1], coord[2], pdbrec.atomic_num, pdbrec.temp_fact,
                                   vox_sz_angst, pdbrec.occ);
                    if (nh) {
                        add_atomic_pot(&(p->pot_re), coord[0], coord[1], coord[2], 1, pdbrec.temp_fact, vox_sz_angst,
                                       nh * pdbrec.occ);
                    }
                    if (p->use_imag_pot) {
    ...
    write_log_comment("PDB coordinates of map center: (%f, %f, %f)\n", (float) (box[0] + 0.5 * (nx - 1) * vox_sz_angst),
                      (float) (box[2] + 0.5 * (ny - 1) * vox_sz_angst),
                      (float) (box[4] + 0.5 * (nz - 1) * vox_sz_angst));
...
int add_atomic_pot(array *vol, double xc, double yc, double zc, int atomic_num, double temp_fact, double voxel_size,
    ...
    for (i = 0; i < NUM_SCATTERING_PARAMETERS; i++) {
        ...
        a[i] *= 4 * sqrt(M_PI) * pow(b[i], -1.5) * PLANCKS_CONSTANT_SQ / (ELECTRON_MASS * ELEMENTARY_CHARGE) /
                (ONE_ANGSTROM_SQ * POTENTIAL_UNIT);
        b[i] = 4 * M_PI * M_PI / b[i];
...
int add_atomic_abs_pot(array *vol, double xc, double yc, double zc, int atomic_num, double temp_fact,
    ...
    double a = PLANCKS_CONSTANT_SQ /
               (8 * M_PI * M_PI * ELECTRON_MASS * ELEMENTARY_CHARGE * ONE_ANGSTROM_CU * POTENTIAL_UNIT)
...
int
get_scattering_parameters(double a[NUM_SCATTERING_PARAMETERS], double b[NUM_SCATTERING_PARAMETERS], int atomic_num) {
    ...
    while (!feof(fp)) {
        ...
        if (parse_pdb_line(line, &p)) {
            ...
            for (i = 0; i < transf->size[2]; i++) {
                ...
                box[0] = min_d(box[0], coord[0]);
                box[1] = max_d(box[1], coord[0]);
                box[2] = min_d(box[2], coord[1]);
                box[3] = max_d(box[3], coord[1]);
                box[4] = min_d(box[4], coord[2]);
                box[5] = max_d(box[5], coord[2]);
            }
        }
    }
    fclose(fp);
