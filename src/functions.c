...
void vecf2d_add(const vecf2d *a, vecf2d *b) {
    ...
    for (k = 0; k < a->values.size[0]; k++) {
        ...
        x0 = 0;
        y0 = coeff[0][2] + 0.5 * (jmin + jmax - b->values.size[1 + dimsb[0]]) * coeff[0][0] +
             (imin - 0.5 * a->values.size[1 + dimsa[1]]) * coeff[0][1];
        for (i = imin; i < imax; i++) {
            interp_add(pd, a->values.size[1 + dimsa[0]], stepsa[dimsa[0]], 1, x0, px, jmax - jmin, imax - imin,
                       coeff[0][0], y0);
            y0 += coeff[0][1];
            pd += stepsa[dimsa[1]];
            px++;
