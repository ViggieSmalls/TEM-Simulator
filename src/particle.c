...
param_table *particle_param_table(const char *name) {
    ...
    add_param_opt_constr(pt, PAR_NZ, "l", 1, 1e4);
    ...
    add_param_def(pt, PAR_MAP_FILE_FORMAT,
                  "s," PAR_FILE_FORMAT__MRC "," PAR_FILE_FORMAT__MRC_INT "," PAR_FILE_FORMAT__RAW,
                  PAR_FILE_FORMAT__MRC);
    add_param_def(pt, PAR_MAP_AXIS_ORDER, "s,xyz,xzy,yxz,yzx,zxy,zyx", "xyz");
    add_param_def(pt, PAR_MAP_FILE_BYTE_ORDER,
                  "s," PAR_BYTE_ORDER__BE "," PAR_BYTE_ORDER__LE "," PAR_BYTE_ORDER__NATIVE,
...
int particle_init(particle *p, simulation *sim) {
    ...
    if (0 == strcmp(source, PAR_SOURCE__MAP)) {
        ...
        generate_random_particle(&(p->pot_re), get_param_double(p->param, PAR_CONTRAST_RE),
                                 get_param_double(p->param, PAR_SMOOTHNESS),
                                 get_param_boolean(p->param, PAR_MAKE_POSITIVE));
        if (p->use_imag_pot) {
    ...
    if (0 == strcmp(source, PAR_SOURCE__MAP)) {
        if (0 == strcmp(PAR_FILE_FORMAT__RAW, get_param_string(p->param, PAR_MAP_FILE_FORMAT))) {
            if (require_param(p->param, PAR_NX) || require_param(p->param, PAR_NY) || require_param(p->param, PAR_NZ)
                || require_param(p->param, PAR_VOXEL_SIZE) || require_param(p->param, PAR_MAP_FILE_RE_IN))
                return 1;
        }
        if (use_imag_pot) {
...
int particle_write_map(particle *p, array *a, const char *fn) {
    ...
    if (0 == strcmp(format, PAR_FILE_FORMAT__RAW)) {
        ...
        ret = write_array_int2b(a, fn, mrc_header, map_axis_order, p->rev_byte_order, voxel_size,
                                POTENTIAL_UNIT / INT_FILE_POT_UNIT);
    } else {
        WARNING("Unknown file format %s.\n", format);
        return 1;
    }
...
int particle_project(particle *p, wavefunction *wf, matrix *proj_matrix, double pos[3]) {
    ...
    steps[0] = 1;
    steps[1] = p->pot_re.size[0];
    steps[2] = (p->pot_re.size[0]) * (p->pot_re.size[1]);
    step0 = steps[dim[0]];
    ...
    for (j = 0; j < 2; j++) {
        ...
        for (i = 0; i < 3; i++) {
            a += 0.5 * p->pot_re.size[i] * voxel_size
                 * fabs(wf->phase.basis[1 + 2 * j] * get_matrix_entry(pm, 0, i) -
                        wf->phase.basis[2 * j] * get_matrix_entry(pm, 1, i));
        }
        b = fabs(wf->phase.basis[1 + 2 * j] * (pos[0] - wf->phase.offset[0]) -
                 wf->phase.basis[2 * j] * (pos[1] - wf->phase.offset[1]));
        c = 0.5 * wf->phase.values.size[2 - j] *
            fabs(wf->phase.basis[0] * wf->phase.basis[3] - wf->phase.basis[1] * wf->phase.basis[2]);
        if (b > a + c) {
