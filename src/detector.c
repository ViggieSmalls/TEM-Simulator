...
param_table *detector_param_table(const char *name) {
    ...
    add_param_def(pt, PAR_IMAGE_FILE_FORMAT,
    ...
    add_param_def(pt, PAR_IMAGE_FILE_BYTE_ORDER,
...
int detector_write_image(detector *d) {
    ...
    if (1 == d->file_header.size[2]) { /* min and max are slightly different for first image. */
        ...
        d->file_header.amean =
                ((d->file_header.size[2] - 1) * d->file_header.amean + min_max_mean[2]) / d->file_header.size[2];
    }
